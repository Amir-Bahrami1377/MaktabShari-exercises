from random import randint
import pickle
from models import Quiz


def load_pickle():
#فایل مجموعه سوالات رو لود میکنه
    with open('questions.pk', 'rb') as f:
        return pickle.load(f)


#به صورت رندوم یک مجموعه انتخاب میشه و لیست ۵تایی سوال جواب استخراج میشه
random_number = randint(0, 2)
question_series: list = load_pickle()
question_lists: list = question_series[random_number]


def data_set():
#ابجکت ها تک تک از لیست خارج و در کلاس فراخوانی میشوند
    question = question_lists.pop(0)
    new_obj = Quiz(question[0], question[1])
    return new_obj


def starting_menu():
    pass

